...
bool isCoordoneesValide(const Grille &grille, unsigned short int ligne,
        ...
        if ((getTailleGrille(grille) >= ligne && ligne > 0) &&
            (getTailleGrille(grille) >= colonne && colonne > 0)) {
                return True;
        }
        return False;
}
...
bool isCaseVide(const Grille &grille, unsigned short int ligne,
                unsigned short int colonne) {
        if ((getSymboleCase(grille, ligne, colonne) ==
             getSymboleCaseVide(grille)) &&
            isCoordoneesValide(grille, ligne, colonne)) {
                return True;
        }
        return False;
}
